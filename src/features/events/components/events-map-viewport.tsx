...
export function EventsMapViewport({ items }: EventsMapViewportProps) {
  ...
  const filteredEvents = useMemo(() => {
    return items.filter((e) => {
      ...
      if (theaterFilter !== 'all' && e.theater !== theaterFilter) return false
  ...
  // Якщо обрали івент → летимо в його точку
  const focusEvent = useMemo(
    () =>
      filteredEvents.find(
        (e) =>
          e.id === selectedEventId && e.latitude != null && e.longitude != null
      ),
